...
interface ProductProps {
  ...
  export function ProductTable ({ productName, variant, image, count, price, onIncrement, onDecrement,checked,onCheck, disabled}:ProductProps) {
    const confirm = useConfirm()
    return (
      <TableRow className="text-2xl">
              <TableCell>
                <div className="flex flex-row gap-2 px-1 py-1">
                  <Checkbox className="h-[25px] w-[25px] mr-2 mt-10 border-blue-500 data-[state=checked]:bg-blue-500" checked={checked} onCheckedChange={onCheck} disabled={disabled}/>
                  <img
                    src={image}
                    alt=""
