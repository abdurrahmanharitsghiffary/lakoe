...
export function CartList() {
  const cartId = useGetCartCollectionId();
  ...
  const handleStoreCheck = (storeId: number) => {
    setStores((prevStores) => {
      ...
      const anyChecked = newStores.some(
        (store) =>
          store.checked || store.products.some((product) => product.checked)
      );
      return newStores.map((store) => ({
        ...
        disabled:
          anyChecked &&
          !store.checked &&
          !store.products.some((product) => product.checked),
      }));
    });
  };
    ...
    setStores((prevStores) => {
      const newStores = prevStores.map((store) => {
        if (store.id === storeId) {
          ...
          const allProductsChecked = newProducts.every(
      ...
      const anyChecked = newStores.some((store) =>
        store.products.some((product) => product.checked)
      );
      return newStores.map((store) => ({
        ...store,
  ...
  const handleCheckout = () => {
    ...
    const totalProducts = checkedItems.reduce(
  ...
  return (
    <div className="flex flex-col w-full px-6">
      ...
      <div className="flex flex-row">
        ...
        <div className="w-[35%]">
          <Card className="rounded-xl border-blue-500 mt-6">
            <Button
              className="border-b-2 bg-transparent text-black rounded-b-none w-full flex justify-between py-8"
          ...
          <Card className="rounded-xl mt-6 border-blue-500">
          ...
          <Button
            className="bg-blue-500 w-full text-xl py-6 my-4 mt-5 drop-shadow-lg hover:bg-blue-400 space-x-3 "
